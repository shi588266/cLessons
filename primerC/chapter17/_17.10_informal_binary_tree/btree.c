...
bool AddItem(const Item *pi, Tree *ptree)
{
    ...
    if(NULL != seek_st.item_node_p) {
        fprintf(stderr, "Error:Duplicated add Error,the item is already exists.\n");
        return false;
    }
...
{
    // 利用 SeekItem 方法
    return SeekItem(pi, ptree).item_node_p == NULL ? true : false;
}
/*
从树中删除一个项
...
static bool ToLeft(const Item *item1, const Item *item2)
{
    ...
    if (cmp_res < 0)
        ...
        //再比较种类
        cmp_res = strcmp(item1->petkind, item2->petkind);
        if (cmp_res < 0)
            return true;
    }
    return false;
